...
import utils
from process import GstElement
...
class AudioInput(InputElement):
    def __init__(self, description, device_location, **kwargs):
        ...
        self.gstelement = self.create_gstelement(self.device_location,
                                                 **kwargs)
    ...
    def _create_audio_source(self, device, **kwargs):
        if not device:
            raise DeviceMissing
        _gstelement = GstElement("pulsesrc", "microphone", **kwargs)
        if not _gstelement:
...
class VideoInput(InputElement):
    def __init__(self, description, communication, device_location, **kwargs):
        ...
        self.device_location = device_location
        self.gstelement = self.create_gstelement(self.device_location, **kwargs)

    def create_gstelement(self, device_location, **kwargs):
        if self.communication == "usb":
            _gstelement = self._create_usbcam_input(device_location, **kwargs)
        elif (self.communication == "tcp"
              or self.communication == "udp"
              or self.communication == "http"
              or self.communication == "tls"):
            _gstelement = self._create_rtsp_input(device_location, **kwargs)
    ...
    def _create_usbcam_input(self, device, **kwargs):
        if not device:
            raise DeviceMissing
        _gstelement = GstElement("v4l2src", "usb_camera", **kwargs)
        if not _gstelement:
            raise GstElementInitError
        _gstelement.set_property("device", device)
...
class StreamElement(OutputElement):
    def __init__(self, description, ip, mount,
                 port=None, password=None, **kwargs):
        OutputElement.__init__(self, description)
        self.ip = ip
        self.mount = mount
        self.port = port
        self.password = password
        self.gstelement = self.create_gstelement(self.ip,
                                                 self.port,
                                                 self.mount,
                                                 self.password,
                                                 **kwargs)
        ...
        def create_gstelement(self, ip, port, mount, password, **kwargs):
            ...
            _gstelement = GstElement("shout2send", "stream_sink", **kwargs)
            if not _gstelement:
                raise GstElementInitError
...
class StoreElement(OutputElement):
    def __init__(self, description, path, **kwargs):
        ...
        def create_gstelement(self, path, **kwargs):
            ...
            _gstelement = GstElement("filesink", "store_sink", **kwargs)
            if not _gstelement:
                raise GstElementInitError
